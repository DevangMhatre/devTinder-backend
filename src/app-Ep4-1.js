...
// ! Fuck this shit.
app.get(/^\/ab?c$/, (req, res) => {
  // * To use v4 type in v5 we need to avoid using "" and add Regex -> /^ $/ and between them v4 routing pattern
  // ?  ^ --> Start of the URL
  // ?  $ --> End of the URL
  // ! if we just use Normal Regexp -> / / then it will even match for something like this -> http://localhost:3000/asda/abc/asda
  // ! to only match http://localhost:3000/abc or http://localhost:3000/ac we need to use /^ $/
  res.send({ firstName: "Devang", lastName: "Mhatre" });
});
...
//   // * bc is optional now since we grouped bc together using ()
//   res.send({ firstName: "Devang", lastName: "Mhatre" });
// });
